...
class Model_params:
    ...
    def get_optimizer(self, tf_learning_rate, loss_tf, label_idx=-1):
        ...
        if  self.mod_type == Model_type.VGG16:
            # If the model is the original VGG16
            grads_and_vars = [
                (gv[0], gv[1]) 
                if ('fc6' in gv[1].name 
                    or 'fc7' in gv[1].name 
        ...
        elif "_W_S" in self.mod_type: 
            # If the model is the original VGG16-CAM
            grads_and_vars = [
                (gv[0], gv[1]) 
    ...
    def get_loss(self, tf_output, tf_labels, conv6=None):
        ...
        if self.l2_weight > 0 :
            weights_only  = [
                ...
                if x.name.endswith('W:0') and "conv6" not in x.name
            ]
            weight_decay  = tf.reduce_mean(tf.pack([
                                tf.nn.l2_loss(x) 
                                for x in weights_only
                            ])) * self.l2_weight
